...
class EcoflowMQTT:
    ...
    def connect(self):
        ...
        log.info(
            f"Connecting to MQTT Broker {self.addr}:{self.port} using client id {self.client_id}"
        )
    ...
    def idle_reconnect(self):
        if (
            ...
            and time.time() - self.last_message_time > self.timeout_seconds
        ):
            log.error(
                f"No messages received for {self.timeout_seconds} seconds. Reconnecting to MQTT"
            )
            ...
            while True:
                ...
                if connect_process.exitcode == 0:
                    ...
                    log.error(
                        "Reconnection errored out, or timed out, attempted to reconnect..."
                    )
...
class EcoflowMetric:
    ...
    def convert_ecoflow_key_to_prometheus_name(self):
        ...
        if not re.match("[a-zA-Z_:][a-zA-Z0-9_:]*", new):
            raise EcoflowMetricException(
                f"Cannot convert payload key {self.ecoflow_payload_key} to comply with the Prometheus data model. Please, raise an issue!"
            )
        return new
    ...
    def process_payload(self, params):
        ...
        for ecoflow_payload_key in params.keys():
            ...
            if isinstance(ecoflow_payload_value, list):
                log.warning(
                    f"Skipping unsupported metric {ecoflow_payload_key}: {ecoflow_payload_value}"
                )
                continue
            ...
            if not metric:
                try:
                    ...
                    continue
                log.info(
                    f"Created new metric from payload key {metric.ecoflow_payload_key} -> {metric.name}"
                )
            ...
            if ecoflow_payload_key == "inv.acInVol" and ecoflow_payload_value == 0:
                ...
                if ac_in_current:
                    log.debug(
                        "Set AC inverter input current to zero because of zero inverter voltage"
                    )
                    ac_in_current.set(0)
...
def start(db, model, baseModel):
    ...
    ecoflow_username = os.getenv("ECOFLOW_USERNAME", "shuhrat.talukder@gmail.com")
    ecoflow_password = os.getenv("ECOFLOW_PASSWORD", "101504Ab#")
    exporter_port = int(os.getenv("EXPORTER_PORT", "9090"))
    collecting_interval_seconds = int(os.getenv("COLLECTING_INTERVAL", "5"))
    timeout_seconds = int(os.getenv("MQTT_TIMEOUT", "60"))

    if not device_sn or not ecoflow_username or not ecoflow_password:
        log.error(
            "Please, provide all required environment variables: DEVICE_SN, ECOFLOW_USERNAME, ECOFLOW_PASSWORD"
        )
        sys.exit(1)
